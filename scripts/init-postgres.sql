...
-- Time bucket concept - We can use time buckets to aggregate metrics over specific time intervals (e.g., 1 minute, 5 minutes, etc.). This allows us to efficiently store and query metrics data without having to store every single request. For example, if we choose a 1-minute time bucket, all requests that occur within the same minute will be aggregated together in a single record.
